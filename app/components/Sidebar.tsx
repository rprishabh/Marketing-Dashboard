...
export default function Sidebar() {
  ...
  const { sidebarOpen, setSidebarOpen } = useBusinessModel();
  ...
  return (
    <>
      ...
      {sidebarOpen && (
        <div
          className="fixed inset-0 bg-black/70 backdrop-blur-md z-30 lg:hidden transition-all duration-300"
      ...
      >
        ...
        <div className="p-5 border-b border-fuchsia-500/10">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              onClick={() => setSidebarOpen(false)}
              className="lg:hidden p-1.5 rounded-lg border border-pink-500/20 bg-slate-900/60 hover:bg-pink-500/10 text-pink-400 transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
